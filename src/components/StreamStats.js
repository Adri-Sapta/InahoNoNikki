...
function StreamStats({ isActive, hasBeenViewed }) {
  ...
  // 3. State baru untuk menyimpan data yang sudah diformat untuk grafik
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
    // 4. Proses data untuk grafik setiap kali filter berubah
    const reversedStreams = [...streams].reverse(); // Balik urutan agar dari terlama ke terbaru
    setChartData({
      ...
      datasets: [
        {
          ...
          borderColor: '#ec4899', // Warna pink terang (Tailwind pink-500)
  ...
  const paginate = (pageNumber) => {
    setIsPaginating(true); // Mulai animasi fade-out
    setTimeout(() => {
      setCurrentPage(pageNumber); // Ganti halaman setelah jeda
      setIsPaginating(false); // Selesaikan animasi (fade-in)
    }, 300); // Durasi jeda harus cocok dengan durasi transisi di CSS
  };

  const chartOptions = {
    responsive: true,
    plugins: { legend: { position: 'top' } },
    scales: { y: { ticks: { callback: value => value >= 1000 ? `${value / 1000}K` : value } } }
  };
  ...
  return (
    <div className={`transition-all duration-700 ease-in-out ${hasBeenViewed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'} ${isActive ? 'animate-pulse-active' : ''}`}>
      <div className="bg-white p-6 rounded-xl shadow-lg border border-pink-100 space-y-8">
        ...
        {/* --- Area Grafik & Tabel Data --- */}
        {loading && <p className="text-center text-gray-500">Memuat statistik...</p>}
        {error && <p className="text-center text-red-500">Error: {error}</p>}
        {!loading && !error && (
          <div className="space-y-8">
            {/* 5. Tampilkan grafik di sini, di atas tabel */}
            {filteredStreams.length > 0 ? (
              <div className="p-4 border rounded-lg bg-pink-50/50">
                 <h3 className="text-lg font-semibold text-gray-800 mb-4 text-center">Tren Views per Stream</h3>
                       ...
                       <tr key={stream.id} className="hover:bg-pink-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="flex-shrink-0 h-10 w-10"><img className="h-10 w-10 rounded-full" src={stream.snippet.thumbnails.default.url} alt="" /></div>
                            <div className="ml-4"><div className="text-sm font-medium text-gray-900 truncate" style={{maxWidth: '250px'}}>{stream.snippet.title}</div></div>
                          </div>
                        </td>
